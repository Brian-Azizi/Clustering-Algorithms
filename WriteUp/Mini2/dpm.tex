...
We used a weakly informative data-dependent prior, as suggested in \cite{Murphy}:
\begin{equation}
\begin{split}
...
[Still working on this. Possible demos (inspired by Murphy book and literature on DPM):
\begin{itemize}
\item Artificial 2D dataset of size 1000, say : show output after training model on 100, 200, 500 and 1000 datapoints (data added sequentially) $\rightarrow$ shows off how \#clusters grows with size of training set.
\item Posterior distribution for $K$ (histograms)
\item Image compression - automatically infer \#clusters. Possibly for different values of $\alpha$.
\item Artificial 2D data: Show output after $L$ iterations of Gibbs sampler, for different $L$.
\end{itemize}
Slightly confused about how to compare speed against Gaussian mixture model or KMeans. There is no clear convergence criterion. Plus, there is probably a lot of obvious optimization that can be done. Perhaps I should try to estimate the complexity? Or try to measure training speed against sample size or dimensionality?]  
