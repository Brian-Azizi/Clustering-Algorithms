...
First, we sample $\bs\Sigma$ from the inverse Wishart distribution, $\bs\Sigma \sim \mathcal{IW}(\bs S,\nu)$.
Then, given $\bs\Sigma$, we sample $\bs\mu$ from the Normal distribution, $\bs\mu \sim \mathcal{N}(\bs m,\frac{1}{\kappa}\bs\Sigma)$.
