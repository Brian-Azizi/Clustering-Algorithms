...
We have implemented the Dirichlet process mixture model in the C++ programming language.
For our implementation, we chose a Gaussian likelihood model with a full Normal-Inverse-Wishart conjugate prior for the base distribution.

We assume that our data lives in $D$-dimensional Euclidean space, $x \in \mathbb{R}^D$ and has a Gaussian distribution parameterised by a mean $\bs\mu$ and a covariance matrix $\bs\Sigma$ so that $\theta = (\bs \mu,\bs\Sigma)$.
The likelihood of a single datapoint $x$ is given by
\begin{equation}
\begin{split}
f(x|\,\theta) &= \mathcal{N}(x|\,\bs\mu,\bs\Sigma)\\
&= (2\pi)^{-\frac{1}{2}D}|\Sigma|^{-\frac{1}{2}} \exp \left((\mu-x)^T \Sigma^{-1} (\mu-x) \right)
\end{split}
\end{equation}

The conjugate prior for $(\bs\mu,\bs\Sigma)$ is the Normal-Inverse-Wishart distribution.
We select this distribution as base measure in the Dirichlet process.
It has four hyperparameters $(\bs m_0,\kappa_0,\bs S_0,\nu_0)$ and its probability density function is given by
\begin{equation}
\begin{split}
H(\theta) &= \mathcal{NIW}(\bs\mu,\bs\Sigma|\,\bs m_0,\kappa_0,\bs S_0,\nu_0)\\
&=Z_{NIW}|\bs\Sigma|^{-\frac{\nu_0+D+2}{2}} \,\times \\
&\exp\left(-\frac{\kappa_0}{2}(\bs\mu-\bs m_0)^T\bs\Sigma^{-1}(\bs\mu-\bs m_0) -\frac{1}{2}\Tr(\bs\Sigma^{-1}\bs S_0) \right)
%&=\mathcal{N}\left(\bs\mu|\,\bs m_0, \frac{1}{\kappa_0}\Sigma\right)\mathcal{IW}\left(\bs \Sigma|\,\bs S_0, \nu_0\right)
\end{split}
\end{equation}
